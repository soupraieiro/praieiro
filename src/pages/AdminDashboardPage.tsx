...
export default function AdminDashboardPage() {
  ...
  const checkAdminAndLoadData = async () => {
    try {
      ...
      // Using user_roles (constitutional) with profile_id
      const { data: roleData } = await (supabase as any).from("user_roles").select("role").eq("user_id", session.user.id).eq("role", "admin").maybeSingle();
